...
int main() {
    ...
    Genome x = Genome("x", "gatgatgat");
    GenomeMatcher* matcher = new GenomeMatcher(3);
    
    matcher->addGenome(x);
    
    vector<GenomeMatch> results;
    cout << matcher->findRelatedGenomes(Genome("a", "gat"), 3, true, 10, results) << endl;
...
const string PROVIDED_DIR = "/Users/joshuavendrow/Documents/cs32/Project\ 4/data";
...
//  Project 4
//
//  Created by Joshua Vendrow on 3/7/19.
//  Copyright © 2019 Joshua Vendrow. All rights reserved.
//
...
int main() {
    ...
    trie.insert("ACGAATCACGA",    42);        //    GATTACA    à {42}
    ...
    std::vector<int> a = trie.find("ACGAATCACGT", true);
    std::cout << a.size();
